...
### v1.5.6 (New)
- **1080P 下方空白修正**：修復自 4K 高解析度切換至 1080P 時下方出現大片多餘空白的問題，啟動時根據寬度與版面動態重新校正高度。
- **WndProc 比例鎖定強化**：底層 Windows 訊息處理常式攔截 `WM_SIZING` 與 `WM_DPICHANGED`，無論滑鼠拉伸或解析度切換皆始終保持精準長寬比。
- **前端智慧高度自動校正（Snap）**：視窗縮放後自動偵測並校正微小高度偏差，避免任何多餘邊界。
