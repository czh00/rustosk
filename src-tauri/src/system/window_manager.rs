...
use crate::system::input_detector::{update_osk_state, get_process_name, get_window_class};
use std::fs;
...
pub fn save_user_manual_position(hwnd: HWND) {
    unsafe {
        ...
        if windows::Win32::UI::WindowsAndMessaging::GetWindowRect(hwnd, &mut rect).is_ok() {
            if let Ok(mut pos) = USER_MANUAL_POS.lock() {
...
#[tauri::command]
pub fn stop_macro() {
    MACRO_STOP_REQUESTED.store(true, Ordering::SeqCst);
}
...
#[tauri::command]
pub fn execute_app(window: tauri::WebviewWindow, cmd: String, start_vk: Option<u16>) {
    let _ = std::thread::spawn(move || {
        // 判定是否為巨集格式 (0xXX, wXXXX, ...)
        if cmd.contains(',') && (cmd.contains("0x") || cmd.contains(",w") || cmd.contains(",l")) {
            // 防止巨集重複執行衝突
            if IS_MACRO_RUNNING.swap(true, Ordering::SeqCst) {
                return;
            }
            MACRO_STOP_REQUESTED.store(false, Ordering::SeqCst);

            let _ = window.emit("macro_status", "🎬 開始執行巨集");
            if let Some(vk) = start_vk {
                let _ = window.emit("macro_loop_status", serde_json::json!({ "active": true, "vk": vk }));
            }
            ...
            for (index, p) in parts.iter().enumerate() {
                ...
                if p_lower.starts_with('l') {
                    ...
                    } else if let Ok(num) = p_lower[1..].parse::<u32>() {
                        if num > 1 {
                            let current_batch = resolved_parts.clone();
                            for _ in 0..(num - 1) {
                                resolved_parts.extend(current_batch.clone());
                            }
                        }
                    }
                } else {
                    resolved_parts.push(p.clone());
                }
            }

            let mut current_iteration = 0;
            loop {
                if MACRO_STOP_REQUESTED.load(Ordering::SeqCst) {
                    let _ = window.emit("macro_status", "🛑 巨集已手動中斷");
                    break;
                }

                if !is_infinite && current_iteration >= 1 {
                    break;
                }

                let total_parts = resolved_parts.len();
                for (index, p) in resolved_parts.iter().enumerate() {
                    if MACRO_STOP_REQUESTED.load(Ordering::SeqCst) {
                        break;
                    }

                    let iter_info = if is_infinite {
                        format!(" (無限循環: 第 {} 次)", current_iteration + 1)
                    } else {
                        "".to_string()
                    };

                    let progress = format!("🔄 進度: {}/{}{}", index + 1, total_parts, iter_info);
                    ...
                    if p_lower.contains('+') || p_lower.starts_with("0x") {
                        ...
                        if let Ok(ms) = p_lower[1..].parse::<u64>() {
                            ...
                            if actual_ms > 0 {
                                // 分段等待以便及時響應中斷
                                let wait_chunks = (actual_ms / 50) as u64;
                                let remainder = (actual_ms % 50) as u64;
                                for _ in 0..wait_chunks {
                                    if MACRO_STOP_REQUESTED.load(Ordering::SeqCst) { break; }
                                    std::thread::sleep(std::time::Duration::from_millis(50));
                                }
                                if !MACRO_STOP_REQUESTED.load(Ordering::SeqCst) && remainder > 0 {
            ...
            let _ = window.emit("macro_loop_status", serde_json::json!({ "active": false }));
...
pub fn adjust_osk_position_for_avoidance(hwnd: HWND) -> bool {
    unsafe {
        let mut osk_rect = RECT::default();
        if windows::Win32::UI::WindowsAndMessaging::GetWindowRect(hwnd, &mut osk_rect).is_err() {
            return false;
        }
        let osk_width = osk_rect.right - osk_rect.left;
...
pub fn show_osk_no_activate() {
    ...
    if hwnd_ptr != 0 {
        unsafe {
            ...
            // 實作自動避讓 (Avoidance) 邏輯
            // 無論是否在固定模式 (IS_PINNED) 下，遇到開始選單或搜尋時都應該避讓
            let is_search_or_start = adjust_osk_position_for_avoidance(hwnd);
            ...
            // 搜尋與開始選單強制置頂，否則依據 temp_no_top 決定
            let should_be_topmost = is_search_or_start || !is_temp_no_top;
            ...
            if !IsWindowVisible(hwnd).as_bool() {
                ...
                if is_currently_topmost != should_be_topmost {
                    let target_z = if should_be_topmost {
                        HWND_TOPMOST
                    } else {
                        HWND(-2 as _)
                    };
...
pub fn resize_and_recenter(
    ...
    if let Ok(handle) = window.hwnd() {
        save_user_manual_position(HWND(handle.0 as _));
    }
    Ok(())
}
