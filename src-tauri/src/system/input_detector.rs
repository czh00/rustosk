...
use windows::Win32::UI::WindowsAndMessaging::{
    GetForegroundWindow, GetGUIThreadInfo, GetWindowThreadProcessId, GUI_CARETBLINKING,
    EVENT_SYSTEM_FOREGROUND, WINEVENT_OUTOFCONTEXT, GetClassNameW,
    EVENT_OBJECT_FOCUS, GetWindowTextW, GUITHREADINFO, SendMessageW, WM_IME_CONTROL,
};
use windows::Win32::UI::Input::Ime::{
    ImmGetContext, ImmGetOpenStatus, ImmReleaseContext,
    ImmGetDefaultIMEWnd, ImmGetConversionStatus, IME_CMODE_NATIVE,
    IME_CONVERSION_MODE, IME_SENTENCE_MODE,
};
use windows::Win32::Foundation::{HWND, BOOL, WPARAM, LPARAM};
use std::time::Duration;
use crate::system::window_manager::{show_osk_no_activate, hide_osk, IS_PINNED, IS_MANUALLY_HIDDEN};
use std::sync::atomic::{Ordering, AtomicUsize};
use windows::Win32::System::Com::{CoInitializeEx, CoUninitialize, COINIT_MULTITHREADED};
use windows::Win32::UI::Accessibility::{
    SetWinEventHook, HWINEVENTHOOK, CUIAutomation, IUIAutomation,
...
lazy_static! {
    ...
    static ref BOOT_TIME: std::time::Instant = std::time::Instant::now();
}
...
pub fn get_process_name(hwnd: HWND) -> String {
    unsafe {
        ...
        if let Ok(h_proc) = h_proc {
            ...
            if QueryFullProcessImageNameW(h_proc, PROCESS_NAME_WIN32, windows::core::PWSTR(buffer.as_mut_ptr()), &mut size).is_ok() {
...
pub fn check_caret() -> bool {
    unsafe {
        ...
        let mut gui_info = windows::Win32::UI::WindowsAndMessaging::GUITHREADINFO {
            cbSize: std::mem::size_of::<windows::Win32::UI::WindowsAndMessaging::GUITHREADINFO>() as u32,
            ..Default::default()
        };
        ...
        if GetGUIThreadInfo(t_id, &mut gui_info).is_ok() {
            ...
            if !gui_info.hwndCaret.0.is_null() {
                let class_name = get_window_class(h_fore);
                if class_name != "Progman" && class_name != "WorkerW" && class_name != "Shell_TrayWnd" && class_name != "Windows.UI.Core.CoreWindow" {
                    return true;
                }
            }
        }
        ...
        if let Ok(automation) = windows::Win32::System::Com::CoCreateInstance::<_, IUIAutomation>(
            ...
            if let Ok(element) = automation.GetFocusedElement() {
                if let Ok(ctrl_type) = element.CurrentControlType() {
                    ...
                    if is_focus && (ctrl_type == UIA_EditControlTypeId || ctrl_type == UIA_ComboBoxControlTypeId) {
                        result = true;
                    }
                }
            }
...
pub fn update_osk_state() {
    ...
    let is_ime_candidate = fg_class.contains("IME") || fg_class.contains("Candidate") || fg_class == "Windows.UI.Core.CoreWindow";
    ...
    if let Ok(guard) = GLOBAL_WINDOW.lock() {
        if let Some(window) = guard.as_ref() {
            ...
            let is_booting = BOOT_TIME.elapsed() < Duration::from_secs(3); // 3 秒寬限期
